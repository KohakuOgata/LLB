...
*/
SceneBase::SceneBase(IOnSceneChangedListener* _implSceneChanged, const Parameter& _parameter)
: implSceneChanged(_implSceneChanged), sceneChangeFlag(false), popSceneFlag(false), parameterBuf(std::make_unique<Parameter>())
{
	m_objects.clear();
	m_drawObjects.clear();
	m_needSort = false;
...
bool SceneBase::Terminate()
{
	return false;
}

void SceneBase::GoTestScene()
{
	Parameter parameter;
	ChangeScene(SceneID::Test, parameter, false);
}

void SceneBase::ChangeSceneUpdate()
{
	if (sceneChangeFlag)
		implSceneChanged->OnSceneChanged(sceneIDBuf, *parameterBuf, stackCleaarBuf);
	sceneChangeFlag = false;
}

void SceneBase::PopSceneUpdate()
{
	if (popSceneFlag)
		implSceneChanged->OnScenePop();
}
...
void SceneBase::ChangeScene(const SceneID scene, const Parameter& parameter, const bool stackClear)
{
	sceneIDBuf = scene;
	*parameterBuf = parameter;
	stackCleaarBuf = stackClear;
	sceneChangeFlag = true;
}

void SceneBase::PopScene()
{
	popSceneFlag = true;
}
